...
// Tauri YouTube Download logic
const ytLoadBtn = DOM("yt-load-btn");
if (ytLoadBtn) {
    ytLoadBtn.addEventListener("click", async () => {
        const url = DOM("yt-url-input").value;
        if (!url) return alert("Please enter a YouTube link.");
        DOM("upload-btn").innerText = "DOWNLOADING...";
        try {
            const bytesArray = await window.__TAURI__.core.invoke("download_youtube_audio", { url: url });
            const uint8Array = new Uint8Array(bytesArray);
            await processAudioData(uint8Array.buffer, "YouTube Track");
        } catch (err) {
            console.error("YouTube Load Error:", err);
            DOM("upload-btn").innerText = "YT LOAD ERROR";
            alert("Error: " + err);
        }
    });
}
